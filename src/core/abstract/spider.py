...
from ..entites.schemas import MangaSchema, ChapterSchema, MangaDetailSchema, BaseMangaSchema
...
class BaseSpider(ABC):
    ...
    async def get_manga(self, url: str | BaseMangaSchema) -> MangaSchema | None:
        ...
        if response := await self.engine.get(url, 'read'):
            return self.parser.parse_manga(
                response
            )
    ...
    def _test_args(self):
        ...
        if not issubclass(self.find_engine, BaseFindEngine):
            raise TypeError(
                "find_engine должен быть наследуемым BaseFindEngine"
            )