...
from ...core.abstract.parser import BaseMangaParser, BasePageParser, BaseChapterParser, BaseParserMother
...
class MultiMangaParser(BaseMangaParser):
    ...
    def _extract_tags(self, soup: BeautifulSoup) -> dict[str, list[str]]:
        ...
        for container in soup.select('section#tags div.tag-container'):
            ...
            result[tag_name] = [tag.get_text(strip=True) for tag in container.select('a.tag')]
            
        return result
    
    def _extract_chapters(self, soup): #NOTE: Мы возращаем самого себя, так-как изображение уже находятся в soup, но для совместимости идём на костыли
