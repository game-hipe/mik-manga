...
class MultiMangaFindEngine(BaseFindEngine):
    ...
    GENRES_URL = (
        "https://multi-manga.today/f/n.m.tags={query}/sort=date/order=desc/page/{page}/"
    )
    ...
    def _build_page(self):
        if self.find_method == "query":
            return self.FIND_URL.format(page=self.page_now, query=self.query)

        return self.GENRES_URL.format(page=self.page_now, query=",".join(self.query))
